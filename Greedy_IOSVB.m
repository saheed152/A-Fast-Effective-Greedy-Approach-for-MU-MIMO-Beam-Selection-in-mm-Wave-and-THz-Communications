...
for tl = 1:10
    ...
    %ind1 = removeRowsWithValues(ind, [4 8 12 16 20]);
    ...
    ind_first = ind(ind_corr_min,:);
    ind_red = removeRowsWithValues(ind, ind_first);


    R_sumo = zeros(length(SNR),1);
    Max_R = zeros(length(SNR),1);
    Min_corr = 10^9;
    max_sv = sum(Comb_sv(ind_red(1,:)));
    size_big=r_sel*User;    
    kount2=0;
    for j = 1:size(ind_red,1)

        if (sum(Comb_sv(ind_red(j,:)))>0.65*max_sv)
            kount2 = kount2 + 1;
            %Fsel_corr = Fopt_corr(:,ind(j,:));
            %temp_diag = Fsel_corr'*Fsel_corr - diag(diag(Fsel_corr'*Fsel_corr));
            temp_corr_sel = sel_matrix(ind_red(j,:),size_big).*big_Fopt;
            temp_diag = temp_corr_sel - diag(diag(temp_corr_sel));
            Corr_F = norm(temp_diag,'fro');
    ...
    ind_sec = ind_red(ind_corr_min,:);
    ind_tot = [ind_first, ind_sec];
